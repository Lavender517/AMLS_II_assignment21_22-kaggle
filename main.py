...
def test():
    ...
    if args.model == 'BERT':
        ...
        saved_model = torch.load('./models/BERT/ckpt_81.25.model') 
        # Saved_model failed to be uploaded to Github because of file size limitation. U can change model hyperparamete to 'LSTM' or 'CNN' to test the result.
    else:
        test_pp = Preprocess(test_x, args.sen_len, w2v_path=w2v_path)
        embedding = test_pp.make_embedding(load=True)
        test_x = test_pp.sentence_word2idx()
        saved_model = torch.load('./models/CNN/ckpt_79.297.model')
